...
menuItem7 = MenuItem(user_id=1, name="Grilled Cheese Sandwich",
                     description="On texas toast with American Cheese", price="$3.49", course="Entree",
...
menuItem2 = MenuItem(user_id=1, name="Peking Duck",
                     description="A famous duck dish from Beijing that has been "
                                 "prepared since the imperial era. The meat is prized "
                                 "for its thin, crisp skin, with authentic versions of the "
                                 "dish serving mostly the skin and little meat, sliced in front "
...
menuItem6 = MenuItem(user_id=1, name="Ramen",
                     description="a Japanese noodle soup dish. It consists of Chinese-style "
                                 "wheat noodles served in a meat- or (occasionally) fish-based broth, "
                                 "often flavored with soy sauce or miso, and uses toppings such as sliced pork, "
...
menuItem2 = MenuItem(user_id=1, name="Chinese Dumplings",
                     description="a common Chinese dumpling which generally consists of minced meat "
                                 "and finely chopped vegetables wrapped into a piece of dough skin. "
...
menuItem1 = MenuItem(user_id=1, name="Tres Leches Cake",
                     description="Rich, luscious sponge cake soaked in sweet milk and topped "
                                 "with vanilla bean whipped cream and strawberries.",
...
menuItem3 = MenuItem(user_id=1, name="Honey Boba Shaved Snow",
                     description="Milk snow layered with honey boba, jasmine tea jelly, grass jelly, "
...
menuItem1 = MenuItem(user_id=1, name="Lamb Curry",
                     description="Slow cook that thang in a pool of tomatoes, onions and alllll those "
                                 "tasty Indian spices. Mmmm.",
                     price="$9.95", course="Entree", restaurant=restaurant1)
...
menuItem1 = MenuItem(user_id=1, name="Boysenberry Sorbet",
                     description="An unsettlingly huge amount of ripe berries turned into frozen "
...
menuItem3 = MenuItem(user_id=1, name="Morels on toast (seasonal)",
                     description="Wild morel mushrooms fried in butter, served on herbed toast slices", price="$7.50",
...
menuItem4 = MenuItem(user_id=1, name="Tandoori Chicken",
                     description="Chicken marinated in yoghurt and seasoned with a spicy mix "
                                 "(chilli, tamarind among others) and slow cooked in a cylindrical clay "
                                 "or metal oven which gets its heat from burning charcoal.",
...
menuItem1 = MenuItem(user_id=1, name="Super Burrito Al Pastor",
                     description="Marinated Pork, Rice, Beans, Avocado, Cilantro, Salsa, Tortilla", price="$5.95",
                     course="Entree", restaurant=restaurant1)
...
menuItem2 = MenuItem(user_id=1, name="Cachapa",
                     description="Golden brown, corn-based Venezuelan pancake; usually stuffed with "
                                 "queso telita or queso de mano, and possibly lechon. ",
                     price="$7.99", course="Entree", restaurant=restaurant1)
